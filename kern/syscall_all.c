...
int sys_mem_alloc(u_int envid, u_int va, u_int perm) {
  ...
  try(envid2env(envid, &env, 1)); // always valid
...
int sys_exofork(void) {
  ...
  DEBUGK("curenv envid %d\n", curenv->env_id);
  DEBUGK("pass env_alloc in sys_exofork\n");
  /* Step 2: Copy the current Trapframe below 'KSTACKTOP' to the new env's
   * 'env_tf'. */
  /* Exercise 4.9: Your code here. (2/4) */
  memcpy(&e->env_tf, (KSTACKTOP - sizeof(struct Trapframe)),
         sizeof(struct Trapframe));
  DEBUGK("pass memcpy in sys_exofork\n");
  /* Step 3: Set the new env's 'env_tf.regs[2]' to 0 to indicate the
  ...
  for(int i=0;i<=64;i++){ // for signal
      e->env_sigaction[i]=curenv->env_sigaction[i];
      e->running_sig[i]=0;
  }
  e->env_sigset_t.sig[0]=0x0;
  e->env_sigset_t.sig[1]=0x0;
  e->env_sig_top=-1;
  e->env_signal_caller=curenv->env_signal_caller; //继承handler
  return e->env_id;
}
...
int sys_set_env_status(u_int envid, u_int status) {
  ...
  DEBUGK("envid2env in sys_set_env_status\n");
  try(envid2env(envid, &env, 1));
  DEBUGK("envid2env out sys_set_env_status\n");
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
  ...
  if (srcva != 0) {
    ...
    try(page_insert(e->env_pgdir, e->env_asid, p, e->env_ipc_dstva,
                    e->env_ipc_perm));
    // if (sys_mem_map(curenv->env_id, srcva, e->env_id, e->env_ipc_dstva,
    //                 e->env_ipc_perm) != 0) {
    //   return -E_INVAL;
    // }
  }
...
void *syscall_table[MAX_SYSNO] = {
    ...
    [SYS_pop_running_sig] = sys_pop_running_sig,
    [SYS_push_running_sig] = sys_push_running_sig,
};

/* Overview:
 *   Call the function in 'syscall_table' indexed at 'sysno' with arguments
 * from user context and stack.
 *
 * Hint:
