...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va,
  ...
  for (int i = 0; i < size; i += BY2PG) {
    ...
    if (page_insert(pgdir, asid, pa2page(pa + i), (va + i), perm | PTE_V) ==
        -E_NO_MEM) {
      break;
    }
  }
...
void env_init(void) {
   ...
   *
   * Here we first map them into the *template*(模板) page directory
   * 'base_pgdir'. Later in 'env_setup_vm', we will copy them into each
   * 'env_pgdir'.
   */
  struct Page *p;
...
int env_alloc(struct Env **new, u_int parent_id) {
  ...
  TAILQ_INIT(&e->sig_wait_list); //新进程初始化sig_wait_list
   ...
   */
  // e->env_sigaction[0].sa_handler=NULL;
  // e->env_sigaction[0].sa_mask.sig[0]=0;
  // e->env_sigaction[0].sa_mask.sig[1]=0;
  for(int i=0;i<=64;i++){ // for sigaction
...
static int load_icode_mapper(void *data, u_long va, size_t offset, u_int perm,
  ...
  if (src != NULL) {
    ...
    memcpy((void *)(KADDR(page2pa(p)) + offset), src, len);
  }
