...
void *diskaddr(u_int blockno) { /* Exercise 5.6: Your code here. */
...
int map_block(u_int blockno) {
  ...
  return syscall_mem_alloc(0, diskaddr(blockno), PTE_D); //用0表示当前进程
}
...
void unmap_block(u_int blockno) {
  ...
  if (syscall_mem_unmap(0, va) != 0) {
    panic_on("error in unmap_block syscall_mem_unmap\n");
  }
