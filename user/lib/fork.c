...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
  ...
  if (!(perm & PTE_COW)) {
    user_panic("doesn't hava PTE_COW\n");
  }
...
static void duppage(u_int envid, u_int vpn) {
   ...
   * marked as COW yet, then map it as copy-on-write, both in the parent (0) and
   * the child (envid). */
  ...
  if (!(perm & PTE_D)) { //只读页面
    if (syscall_mem_map(0, addr, envid, addr, perm) != 0) {
      user_panic("fail on only read\n");
    }
  } else if (perm & PTE_COW) { // 非共享页面，写时复制
    if (syscall_mem_map(0, addr, envid, addr, perm) != 0) {
      user_panic("fail on COW\n");
    }
  } else if (perm & PTE_LIBRARY) { // 共享页面
    if (syscall_mem_map(0, addr, envid, addr, perm) != 0) {
      user_panic("fail on share\n");
    }
  } else { //可写页面
    perm |= PTE_COW;
    perm ^= PTE_D;
...
int fork(void) {
  ...
  if (child == 0) { //子进程中返回0，然后要把env转化为指向自己的
  ...
  for (i = 0; i < VPN(USTACKTOP); i++) { // vpt是页表的基地址，vpd是页目录基地址
